...
import { useDraggable, useDroppable, useDndContext } from '../../context/dnd-context';
import { cn } from '../../lib/utils';
import { ShoppingCart, Trash2, Search, Plus, Minus, X, Filter } from 'lucide-react';
...
const BoxDroppable = () => {
  const { boxItems, setBoxItems, removeItemFromBox, increaseItemQuantity, decreaseItemQuantity } = useDndContext();
        ...
        {boxItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
            <div className="relative w-48 h-48 mb-4">
              <Image src="/assets/caja-frikibox.png" alt="Caja FrikiBox" fill className="object-contain" />
            </div>
          ...
          <ScrollArea className="whitespace-nowrap">
              <div className="flex gap-4 pb-4">
                {boxItems.map(({product, quantity, id}) => (
                  <div key={id} className="relative aspect-square bg-white rounded-md p-1 group w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0">
                    <Image
                        src={product.imageUrl}
                        alt={product.name}
                        fill
                      ...
                      <Badge variant="secondary" className="absolute top-1 right-1 z-10">
                        {product.selectedSize && `${product.selectedSize} `}x{quantity}
                      </Badge>
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1 z-20">
                        <div className='flex items-center gap-1'>
                          <Button variant="ghost" size="icon" className="h-6 w-6 text-white" onClick={() => decreaseItemQuantity(id)}><Minus className="h-4 w-4"/></Button>
                          <Button variant="ghost" size="icon" className="h-6 w-6 text-white" onClick={() => handleIncreaseQuantity(id)}><Plus className="h-4 w-4"/></Button>
                        </div>
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => removeItemFromBox(id)}><Trash2 className="h-4 w-4"/></Button>
                      </div>
                  </div>
                ))}
              </div>
...
export default function PersonalizePage() {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (selectedFranchise !== 'all') {
        productsToShow = allProducts.filter(p => p.mainFranchise === selectedFranchise);
    }
  ...
  const total = boxItems.reduce((sum, item) => sum + (item.product.price || 0) * item.quantity, 0);
  ...
  const handleAddBoxToCart = () => {
    if (boxItems.length === 0) {
      toast({
        variant: 'destructive',
        title: '¡Caja vacía!',
        description: 'Añade algunos productos a la caja antes de añadirla al carrito.',
      });
      return;
    }
    const customBox: ImagePlaceholder = {
      ...
      description: boxItems.map(p => `${p.product.name}${p.product.selectedSize ? ` (${p.product.selectedSize})` : ''} (x${p.quantity})`).join(', '),
    };
    ...
    <section className="py-12 md:py-16 lg:py-20 flex-grow">
      <div className="container px-4 md:px-6 h-full flex flex-col">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
          <div className="space-y-2">
            <h1 className="text-3xl font-headline font-bold tracking-tighter sm:text-5xl animate-fade-in-down">Crea tu propia FrikiBox</h1>
            <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed animate-fade-in-up">
              Arrastra y suelta tus productos favoritos o haz clic para añadir ropa. Límite de 6 productos por caja.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 flex-grow">
          <div className="md:w-1/3 flex flex-col">
            <h2 className="text-2xl font-headline font-bold mb-4">Productos Disponibles</h2>
            
            <div className="flex flex-col sm:flex-row gap-4 mb-4">
                <div className='relative flex-grow'>
                     <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                     <Select value={selectedFranchise} onValueChange={setSelectedFranchise} disabled={productsLoading}>
            ...
            <div className="grid grid-cols-3 gap-4 overflow-y-auto pr-4 max-h-[50vh] md:max-h-none md:flex-grow">
              {productsLoading ? (
                ...
                <div className='col-span-3 text-center text-muted-foreground py-10'>No se encontraron productos.</div>
              )}
            </div>
          </div>
